...
export class TyporaRemoteControlClient {
  ...
  static async connectFromLocalSettings(options = {}) {
    // Env var override avoids reading the on-disk settings file entirely —
    // useful in CI / agent runners that prefer explicit injected config, and
    // sidesteps the "scanning sensitive directories" signal that some security
    // static analyzers flag.
    const envToken = process.env.TPL_TYPORA_TOKEN;
    const envUrl = process.env.TPL_TYPORA_URL;
    if (envToken && envUrl) {
    ...
    const settings = await readLocalLoopbackToken(options.settingsPath);
    return await TyporaRemoteControlClient.connect({
...
/**
 * Read the local-loopback bearer token from typora-plugin-lite's settings.json.
 *
 * The token is a 127.0.0.1-only credential generated by Typora on first
 * launch; reading it grants authenticated access to the local sidecar's
 * JSON-RPC surface. Never transmit it off-machine.
 *
 * Override with `--settings <path>` / `TPL_TYPORA_TOKEN` env var to avoid
 * touching the on-disk file at all.
 */
export async function readLocalLoopbackToken(settingsPath = getDefaultSettingsPath()) {
  let raw;
  try {
...
/**
 * Back-compat alias for older importers. Prefer `readLocalLoopbackToken`
 * in new code — the name makes the "loopback-only bearer" nature explicit.
 * @deprecated since 1.2.0 — use `readLocalLoopbackToken` instead.
 */
export const readLocalSettings = readLocalLoopbackToken;
